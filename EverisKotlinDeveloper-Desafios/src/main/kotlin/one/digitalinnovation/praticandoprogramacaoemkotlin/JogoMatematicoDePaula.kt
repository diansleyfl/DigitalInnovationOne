...
Seu maior passatempo é ficar inventando jogos ou atividades que a
envolvam para brincar com seus amiguinhos.
Obviamente, nem todos eles não são tão apaixonados assim por
matemática e têm muita dificuldade para resolver
as brincadeiras propostas por ela. Agora Paula inventou um
pequeno passatempo que envolve 3 caracteres:
...
para criar um programa para que encontre a solução para cada
...
fun main(args: Array<String>) {
    ...
    var sequenciaDigitada: List<String>
    ...
    for (i in 1..qtdeTestes) {
        ...
        sequenciaDigitada = readLine()!!.split(" ")

        for (i in sequenciaDigitada) {
            primeiroNumero = i.substring(2, 3).toInt()
            letra = i.substring(1, 2).toString()
            segundoNumero = i.substring(0, 1).toInt()


            if (letra == letra.toUpperCase() && primeiroNumero != segundoNumero){
                operacao = primeiroNumero - segundoNumero
                resultado.add(operacao)
            }

            if (letra == letra.toLowerCase() && primeiroNumero != segundoNumero){
                operacao = primeiroNumero + segundoNumero
                resultado.add(operacao)
            }
